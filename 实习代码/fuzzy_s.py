...
def fuzzy_search(conn, table_name, search_string, column='column_name'):
    ...
    query = f"SELECT * FROM {table_name} WHERE {column} LIKE %s  " \
            ...
            # f" AND CAST(TRADE_DT AS SIGNED) = 20231116"
    ...
    # data = {'S_CON_WINDCODE': parameter_a_list}
    # df = pd.DataFrame(data)
    # # #
    # df.to_csv('./result/new/688成分股.csv', index=False)
...
if __name__ == "__main__":
    ...
    zhongzheng2000 = fuzzy_search(conn, 'AINDEXMEMBERS', '000985.CSI', 'S_INFO_WINDCODE')
    # hushen300 = fuzzy_search(conn, 'AINDEXMEMBERS', '399300.SZ', 'S_INFO_WINDCODE')
    # zhongzheng500 = fuzzy_search(conn, 'AINDEXMEMBERS', '000905.SH', 'S_INFO_WINDCODE')
    # zhongzheng1000 = fuzzy_search(conn, 'AINDEXMEMBERS', '000852.SH', 'S_INFO_WINDCODE')
    # guozheng2000 = fuzzy_search(conn, 'AINDEXMEMBERS', '399303.SZ', 'S_INFO_WINDCODE')
    # combine_list = shang50 + hushen300 + zhongzheng1000 + zhongzheng500 + guozheng2000
    # combine_list = list(set(combine_list))

    # with open('reference/components_miss_diff/zhongzheng2000.csv', 'w', newline='') as file:
    #     writer = csv.writer(file)
    #     writer.writerow(["Stock"])  # 写入标题行
    #     writer.writerows([[stock] for stock in zhongzheng2000])
    #
