...
# @Time: 2023/11/1 9:15
# @Author: Wu You
# @File：double_index_plt.py
# @Desc: 说明：统计指定指定的两个指数的一级行业分布对比
#        行业 - WIND一级行业
#        数据库 - Wind金融数据库
# @Software: PyCharm
...
def plot_two_industry_distributions(conn, category1, category2):
    # 获取两个指数的行业分布
    industry_counts1, all1, name1 = plot_industry_distribution(conn, category1)
    industry_counts2, all2, name2 = plot_industry_distribution(conn, category2)
...
def plot_industry_distribution(conn, category):
    ...
    return industry_counts, all_industries, name_result_str
...
if __name__ == "__main__":
    ...
    # 提示用户输入指数类别
    category1 = input("请输入第一个要绘制行业分布图的指数类别: ")
    category2 = input("请输入第二个要绘制行业分布图的指数类别: ")

    # 绘制行业分布图
    # plot_industry_distribution(conn, category1)
    plot_two_industry_distributions(conn, category1, category2)
