...
def industry_distribution(conn, date, category):
    ...
    return category_counts, name_result_str
...
if __name__ == "__main__":
    ...
    for index, row in df.iterrows():
        ...
        print(index_code, ":")
        a, _, _ = select_data_by_date_index(data_use, index_code)
        df.at[index, f'{data_use}_DangRi'] = round(a, 5)

        # 定义新的列顺序
    new_order = ['category', 'category_chinese', 'count', 'percentages', f'{data_use}_DangRi', 'index_code ']
    # 使用reindex方法对列进行重新排序
    df = df.reindex(columns=new_order)

    selected_columns = ['category', 'category_chinese', 'count', 'percentages', f'{data_use}_DangRi']
    selected_df = df[selected_columns]

    print(selected_df)
    selected_df.to_csv(f'./result/{data_use}_{name}行业详情信息.csv', index=False)
