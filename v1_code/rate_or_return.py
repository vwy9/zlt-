...
# 成分股计算
# 成分股对应行业的收益率
def components_single_date_ewror(conn, date, stock_list):
    # 将股票代码列表转换成单引号包围并由逗号分隔的字符串
    quoted_stocks = "', '".join(stock_list)
    query_step = f"""
        SELECT S_INFO_WINDCODE, S_DQ_ADJPRECLOSE, S_DQ_ADJCLOSE, S_DQ_ADJOPEN
        FROM ASHAREEODPRICES
        WHERE S_INFO_WINDCODE IN ('{quoted_stocks}')
        AND CAST(TRADE_DT AS SIGNED) = {date}
    """
    try:
        # 执行查询
        cursor = conn.cursor()
        ...
        query_step_result = cursor.fetchall()
        columns = [desc[0] for desc in cursor.description]
        industry_ror = pd.DataFrame(query_step_result, columns=columns)
        # 计算收益率的代码可以在这里添加
        # ...
        cursor.close()
        return industry_ror  # 确保返回的是查询结果
    except pymysql.Error as e:
        print(f"执行查询发生错误: {e}")
        return pd.DataFrame()
