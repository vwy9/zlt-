...
def select_data_by_date_index(input_date: int, index_type):
    ...
    try:
        ...
        for i in range(d_trans):
            dangri[i] = t_close[i] / pre_close[i] - 1
            geye[i] = t_open[i] / pre_close[i] - 1
            rinei[i] = t_close[i] / t_open[i] - 1
            i += 1

        dangri_avg = round(sum(dangri) / d_trans, 5)
        geye_avg = round(sum(geye) / d_trans, 5)
        rinei_avg = round(sum(rinei) / d_trans, 5)

        print("当天涨跌平均值为：", dangri_avg)
        print("日内涨跌平均值为：", rinei_avg)
        print("隔夜涨跌平均值为：", geye_avg)
